...
| **2a. Ink DATA cutover** — ✅ **DONE** (2026-07-10) | Legacy `NotePage.drawingData`/`ocrText`/`typedText` migrate into `PageBlock`s on first read (inside the live `fetchBlocksForPage`, idempotent); every canvas save path (lifecycle flush, 50-stroke checkpoint) retargeted from `saveDrawing` to `updateBlockDrawing(inkBlockID,…)` — **zero production writers of the legacy fields remain**; `CanvasScreen` loads block-first and seeds an ink block on fresh pages; the live client mirrors ink thumbnails to the page card so the library grid is unchanged; `bindCanonicalCanvasWidth` fires on first canvas geometry (§6.4 groundwork — render scale stays 1.0 on the authoring device). Legacy fields RETIRED 2026-07-22 as a direct schema edit — per the no-migrations rule the migrate-on-read shim was deleted rather than swept: the store either opens (pages opened post-cutover already hold blocks) or the user reinstalls. | The payload model is per-block BEFORE any view moves — data risk separated from visual risk |
| **2b. Ink VIEW cutover onto the stack** — *needs a device-verified session* | `InkBlockRowView` hosting `PKCanvasView` at canonical width + `scaleEffect`; `.notebook`/`.quickSheet` route onto `PageBlockStackView`; `CanvasScreen` carve-up (toolbar/dismiss chrome stays at `NotebookScreen` level per the pinned-chrome invariant; lasso/dispatch/session machinery re-homed). Deliberately split from 2a: Pencil feel, template alignment, lasso coordinate spaces, and scale rendering are visual behaviors that must be verified on hardware, not headlessly. | The either/or switch dissolves; canvas parity on the new architecture |
