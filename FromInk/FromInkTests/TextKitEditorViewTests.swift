...
final class TextKitEditorViewTests: XCTestCase {
    ...
    // MARK: - Ordered-list ordinal (ParagraphIndex variant)

    func test_orderedItemOrdinal_fromIndex_countsWithinGroup() {
        let doc = RichTextDocument(blocks: [
            Block(kind: .orderedList(items: [
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "one")]))]),
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "two")]))]),
        ...
        let entries = ParagraphIndex(document: doc).entries
        XCTAssertEqual(entries.count, 3)
        for i in 0..<3 {
            XCTAssertEqual(BlockDecoratingLayoutManager.orderedItemOrdinal(in: entries, at: i), i + 1)
        }
    }

    func test_orderedItemOrdinal_fromIndex_restartsAcrossSeparateLists() {
        let doc = RichTextDocument(blocks: [
            Block(kind: .orderedList(items: [
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "a1")]))]),
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "a2")]))])
        ...
        // entries[2] is the interlude paragraph; the second list is 3,4.
        XCTAssertEqual(BlockDecoratingLayoutManager.orderedItemOrdinal(in: entries, at: 3), 1,
                       "Fresh list group restarts at 1 — the walk stops at the interlude paragraph")
        XCTAssertEqual(BlockDecoratingLayoutManager.orderedItemOrdinal(in: entries, at: 4), 2)
    }

    func test_orderedItemOrdinal_indexVariant_matchesStorageVariant() {
        let doc = RichTextDocument(blocks: [
            Block(kind: .orderedList(items: [
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "x1")]))]),
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "x2")]))]),
                ListItem(content: [Block(kind: .paragraph(inline: [Inline(text: "x3")]))])
            ]))
        ])
        let result = TextKitEditorView.flatten(document: doc, bodyFont: bodyFont, bodyColor: bodyColor)
        let entries = ParagraphIndex(document: doc).entries
        ...
        for (i, start) in starts.enumerated() {
            let groupID = result.attributed.attribute(.groupID, at: start, effectiveRange: nil) as? UUID
            let storageOrdinal = BlockDecoratingLayoutManager.orderedItemOrdinal(
                in: result.attributed, paragraphStart: start, groupID: groupID
            )
            let indexOrdinal = BlockDecoratingLayoutManager.orderedItemOrdinal(in: entries, at: i)
            XCTAssertEqual(indexOrdinal, storageOrdinal, "Index ordinal must match the storage walk at item \(i)")
        }
    }

    // MARK: - BlockChrome → ParagraphKind bridge

    func test_blockChrome_paragraphKind_mapping() {
        XCTAssertEqual(BlockChrome.paragraph.paragraphKind, .paragraph)
        XCTAssertEqual(BlockChrome.heading1.paragraphKind, .heading(level: 1))
        XCTAssertEqual(BlockChrome.heading2.paragraphKind, .heading(level: 2))
        XCTAssertEqual(BlockChrome.heading3.paragraphKind, .heading(level: 3))
        XCTAssertEqual(BlockChrome.bulletListItem.paragraphKind, .bulletListItem)
        XCTAssertEqual(BlockChrome.orderedListItem.paragraphKind, .orderedListItem)
        XCTAssertEqual(BlockChrome.blockquoteParagraph.paragraphKind, .blockquoteParagraph)
        XCTAssertEqual(BlockChrome.codeBlock.paragraphKind, .codeBlock)
        XCTAssertEqual(BlockChrome.divider.paragraphKind, .divider)
    }

    func test_layoutManager_newlineCount_inRange() {
        let s = "a\nbb\n\nc\n" as NSString  // paragraphs: "a","bb","","c" + trailing \n
        XCTAssertEqual(BlockDecoratingLayoutManager.newlineCount(in: s, range: NSRange(location: 0, length: s.length)), 4)
        // Prefix up to the start of "bb" (location 2): one \n before it.
        XCTAssertEqual(BlockDecoratingLayoutManager.newlineCount(in: s, range: NSRange(location: 0, length: 2)), 1)
        // Empty range counts nothing.
        XCTAssertEqual(BlockDecoratingLayoutManager.newlineCount(in: s, range: NSRange(location: 0, length: 0)), 0)
    }
